...
JOIN webshops w ON w.id = wp.shop_id
...
**Patching `completed` on buy orders:** After creating BOs, set `completed` (when the BO was closed). Use [build_patch_completed_bodies.js](../retool-blocks/build_patch_completed_bodies.js): input = list of created BOs **with ids** (e.g. the POST buy orders Loop’s response array). BOs with `expectedDeliveryDate` in the future are skipped. For the rest: ~80% get `completed` = `expectedDeliveryDate`; ~20% get `completed` = expectedDeliveryDate + 1–5 days (late). Output: `patch_items` = `[{ id, body }]`. Add a **Loop** that PATCHes each: `PATCH https://api.optiply.com/v1/buyOrders/{{ item.id }}?accountId=1380`, body = `item.body`, same JSON:API header. If you only have request bodies (no ids), GET buy orders for the account and use that list, or pass `bo_ids_in_order` (array of ids in same order).

**Assembly orders Phase 2 (T6 — complete, in-transit, receipt lines):** If you have an assembly-orders path (create assembly orders via Loop, or they already exist in DB), add: (1) **fetch_bo_data_assembly.sql** to load BO/BOL rows for assembly orders. (2) **build_assembly_receipt_line_bodies.js** — input = fetch_bo_data_assembly.data (or fetch_bo_data.data); builds `receipt_line_bodies` from each row (bol_id, expected_delivery_date, quantity). (3) **Loop: POST receipt lines** over `build_assembly_receipt_line_bodies.data.receipt_line_bodies` to the same receiptLines endpoint. (4) **build_patch_completed_bodies** with input = post_assembly_orders_loop.data or fetch_bo_data_assembly.data; same Loop PATCH buy orders. Past-dated assembly BOs get `completed` (on-time or late); future-dated stay **in transit**. See [retool-blocks/README.md](../retool-blocks/README.md) “Assembly orders (Phase 1 + Phase 2)”.
